...
list_species = [Aphelinus.abdominalis, Aphelinus.albipodus, Aphelinus.asychis, Aphelinus.certus, Aphelinus.chaonia,
                ...
                Encarsia.formosa, Encarsia.guadaloupae, Encarsia.haitensis,
                Encarsia.hispida, Encarsia.luteola, Encarsia.meritoria, Encarsia.oakeyensis, Encarsia.protransvena,
                Encarsia.smithi, Encarsia.sophia, Encarsia.strenua, Eretmocerus.debachi, Eretmocerus.eremicus,
...
def out_loc_num(ls=list_species, extremes=False):
    ...
    if not extremes:
        ...
        for k in ls:
            a.write(str(k["gen"]) + " " + str(k["sp"]) + "," + str(dist_area(k, one_sp=False)[0]) + ";" + str(
                dist_area(k, one_sp=False)[1]) + "," + str(dist_area(k, one_sp=False)[2]) + ";" + str(
                dist_area(k, one_sp=False)[3]) + "," + str(len(k["geo"]) / 3) + "\n")
    a.close()
