        ...
        for m in sorted(spe_targ[k][l]):
            ...
            for n in sorted(spe_targ[k][l][m]):
                ...
                whole_data.write(str(k)+','+str(l)+','+str(m)+','+str(n)+','+
                                 get_distribution(spe_targ[k][l][m])[1]+','+
                                 min(get_distribution(spe_targ[k][l][m])[0],key=lambda x: x[0])+','+
                                 min(get_distribution(spe_targ[k][l][m])[0],key=lambda x: x[1])+','+
                                 max(get_distribution(spe_targ[k][l][m])[0],key=lambda x: x[0])+','+
                                 max(get_distribution(spe_targ[k][l][m])[0],key=lambda x: x[1])+'\n')
